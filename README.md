...
### One-time EC2 setup (Ubuntu 22.04+)
...
sudo chown -R ubuntu:ubuntu /srv/enably-admin

# 3. Drop in the systemd unit + env file (see deploy/*.example)
...
sudo chown root:ubuntu /etc/enably-admin.env
sudo systemctl daemon-reload
sudo systemctl enable enably-admin

# 4. Nginx vhost + TLS
sudo cp deploy/nginx.conf.example /etc/nginx/sites-available/enably-admin
sudo ln -s /etc/nginx/sites-available/enably-admin /etc/nginx/sites-enabled/enably-admin
